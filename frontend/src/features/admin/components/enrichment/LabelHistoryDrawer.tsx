...
import type { LabelHistoryCell } from '../../../../api/labels';
...
interface RunGroup {
  ...
  cells: LabelHistoryCell[];
}

function groupByRun(items: LabelHistoryCell[]): RunGroup[] {
...
export function LabelHistoryDrawer({ opened, onClose, labelId, labelName }: Props) {
  const { t } = useTranslation();
  const query = useLabelHistory(labelId);
              ...
              <Accordion.Control>
                <Group justify="space-between" wrap="nowrap">
                  <Group gap="xs" wrap="nowrap">
                    <Anchor
                      component={Link}
                      to={`/admin/labels/enrich/runs/${g.runId}`}
                      size="sm"
...
function CellSection({ cell }: { cell: LabelHistoryCell }) {
  const { t } = useTranslation();
