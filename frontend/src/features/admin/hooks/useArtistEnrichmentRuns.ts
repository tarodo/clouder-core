...
import type { RunsListResponse } from '../../../api/artists';

export interface UseArtistRunsParams {
  status: 'all' | 'queued' | 'running' | 'completed' | 'failed';
  source?: 'manual' | 'auto';
}

export function useArtistEnrichmentRuns(p: UseArtistRunsParams) {
  ...
  return useInfiniteQuery<RunsListResponse, Error>({
    queryKey: ['admin', 'artistEnrichmentRuns', p.status, p.source ?? null] as const,
    queryFn: ({ pageParam }) => {
      ...
      return api<RunsListResponse>(`/admin/artists/enrich-runs?${qs.toString()}`);
    },
