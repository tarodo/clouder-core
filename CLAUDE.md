...
- **Optimistic shrink + auto-advance interaction.** `useMoveTracks.applyOptimisticMove` filters the assigned track out of the bucket-tracks query cache synchronously. Curate's queue (the same query) shrinks by 1 immediately. The reducer's `ADVANCE` action then increments `currentIndex` after 200ms — so the user effectively SKIPS one track (queue [t1,t2,t3] → assign t1 → optimistic to [t2,t3] → 200ms advance to index 1 → shows t3, not t2). This is a known limitation; making `ADVANCE` a no-op would fix it but breaks `useCurateSession` test invariants. Acceptable today because the user mostly notices "next track shown" rather than "specific track shown" in fast curation. Revisit in iter-2b.
