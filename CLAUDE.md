...
- **`LookupProvider` gained per-track methods in Plan 4.** `lookup_by_isrc(isrc) -> VendorTrackRef | None` and `lookup_by_metadata(artist, title, duration_ms, album) -> list[VendorTrackRef]`. Spotify implements ISRC; the per-track `lookup_by_metadata` on `SpotifyLookup` still returns `[]` (separate code path, used by `vendor_match_worker`). All other vendors raise `VendorDisabledError(reason="not_implemented")`.
- **`SpotifyClient` runs three lookups in order on ISRC miss.** When `?q=isrc:` returns 0 items AND `SPOTIFY_METADATA_FALLBACK_ENABLED=true`, the client tries — in order — (1) sibling ISRCs (`±1, ±2` on last digit, closest-first) verified by title+artist gate (NO duration check, since master/radio-edit can differ wildly), then (2) `?q=track:<title> artist:<first_artist>` metadata search where `<first_artist>` = first of comma/&-list with `(UK)` / `(US)` country suffix stripped (Spotify's `artist:` operator substring-matches literally, so passing the BP-shaped `Kays (UK), Nixxy Rain` returns 0 items even when the track exists). Stage 2 has TWO accept tiers: STRICT (`title_sim >= SPOTIFY_FUZZY_TITLE_MIN` default 0.90, `artist_sim >= SPOTIFY_FUZZY_ARTIST_MIN` default 0.85, `|dur_diff| <= SPOTIFY_FUZZY_DURATION_TOLERANCE_MS` default 3000ms) and RELAXED (same min thresholds + `title_sim >= 0.95` + `artist_sim >= 0.95`, no duration check — same track, different master/radio-edit). Strict candidates win when both tiers have hits. Both stages normalize titles via `_normalize_title_for_match` (strips `feat. X`, `(...Mix)`, `- Radio Edit`, `- Extended Mix`, etc.). Disabled by default — set `SPOTIFY_METADATA_FALLBACK_ENABLED=true` on `beatport-prod-spotify-search-worker` to activate. `find_tracks_needing_spotify_search` JOINs `clouder_track_artists`/`clouder_artists` and projects `length_ms` so the worker has the metadata to issue the fallback. Logs: `spotify_isrc_neighbour_match` (stage 1 hit), `spotify_metadata_fallback_attempted` / `_match` (strict) / `_match_relaxed` (tier-2) / `_rejected` (stage 2), `spotify_metadata_fallback_scores` (best title/artist sims observed, emitted on stage-2 reject for tuning).
...
- **`SPOTIFY_OAUTH_REDIRECT_URI` Lambda env (`beatport-prod-auth-handler`) determines the OAuth redirect target.** `auth.tf` coalesces `var.spotify_oauth_redirect_uri` (empty by default) → `https://${aws_cloudfront_distribution.frontend.domain_name}/auth/return`. Set the variable explicitly only for the SPA dev flow (`http://localhost:5173/auth/return`). Setting it to the API GW callback URL breaks login — Spotify lands the user on `*.execute-api.amazonaws.com` directly, the `oauth_state` cookie is bound to the CloudFront domain (different host), so `/auth/callback` always returns `csrf_state_mismatch`. The CloudFront URL must ALSO be added to the Spotify Developer Dashboard Redirect URIs allow-list (manual step, not Terraform-able).
...
- **`just-tapped` is scale-only, no color flash.** Earlier F5 iterations mounted an `accent-magenta` body class to colour the destination button on tap (per design Q7). In practice the magenta flash felt out of place in an otherwise monochrome UI and only landed visibly during slow renders. The class lifecycle was removed; `[data-just-tapped='true']` now applies `transform: scale(0.97)` only. If a future iteration brings the colour back, restore the body-class lifecycle in `CurateSessionPage` and the `background: var(--color-selected-bg)` rule in `DestinationButton.module.css` together — they go as a pair.
...
- **F6: PlaybackProvider lives in the authenticated `_layout.tsx`.** SDK script is lazy-loaded only on the first `controls.play()` call. Public auth pages never instantiate the provider, so they don't request a Spotify token. SDK boot races: first `play()` calls `ensureSdk()` then returns early because `deviceIdRef` is null until the SDK emits `ready`; integration tests must click play twice with an emitted `ready` between (see `integration.batch1.test.tsx` `preWarm()`).
- **F6: `spotify_access_token` is bundled with the CLOUDER auth refresh stream.** Backend already returns it on `/auth/callback` and `/auth/refresh`. SPA stores it in the in-memory `spotifyTokenStore` (mirror of `tokenStore`); never persists to localStorage / sessionStorage / cookies (PB16). SDK reads via `getOAuthToken(cb)` which calls `spotifyTokenStore.get()` synchronously, so token rotation is transparent on next callback invocation.
- **F6: Hotkey swap from F5 ships in this ticket.** `J = prev`, `K = next` (was: `J = skip → next`, `K = prev`). `Space` no longer opens external Spotify (now play/pause via `usePlaybackHotkeys`). The "Open in Spotify" affordance lives as a button-only icon inside `CurateCard` (no hotkey).
- **F6: F5 reducer cursor remains source of truth.** Hybrid model — `useCurateSession` owns `currentIndex`; `PlaybackProvider.bindQueue` rebinds on every tracks-identity change with an `onCursorChange` callback that dispatches `JUMP_TO`. Do not move cursor ownership into the provider.
- **F6: Auto-advance after destination tap calls `playback.controls.play()`, not `next()`.** F5's `ADVANCE` reducer action stays a no-op (CLAUDE.md "Optimistic shrink does the work" gotcha unchanged). At the end of the 200 ms hold, `useCurateSession.scheduleAdvance` fires `playback.controls.play()` against the (already-shrunk) tracks list; the bound cursor naturally points at the new "next" track. Calling `next()` would advance cursor by +1 and skip a track. Undo cancels via `playback.controls.cancelPendingAdvance()` BEFORE the rollback dispatch.
- **F6: Null-spotify-id auto-skip is incomplete.** When the post-shrink track at cursor has `spotify_id == null`, `controls.play()` is a silent no-op (per T14 guard). Auto-advance does NOT currently jump to the next playable forward — `integration.batch1.test.tsx` scenario 4 asserts the broken behaviour with a TODO. Fix path: extend `controls` with a `playFromCursorSkippingNull()` method, OR change `scheduleAdvance` to call a new combined "play-or-advance" entry point.
- **F6: Device picker is auto-pick only.** `transferMyPlayback(device_id, { play: false })` fires once on SDK `ready`. No UI in F6. F7 builds the P-25 picker. PlayerCard's "Open device picker" link in `disconnected` state is a no-op stub in F6.
- **F6: Slider scrub commits via `onChangeEnd`.** Mid-drag `onChange` is debounced 100 ms via `useScrubDebounce` to avoid SDK rate-limiting. Disabled in `error` / `disconnected` / `empty-bucket` states. Mantine 9 `Slider` is undriveable in jsdom — the debounce hook is unit-tested in isolation; end-to-end scrub behaviour is covered by integration tests T31-T34.
- **F6: `useBlocker` only fires when target is another PlayerCard route AND queue is active.** Tracks list / Profile / Home / Triage list / Categories list pass through; MiniBar appears. The route detection uses `routeContext.hasPlayerCard(pathname)` matching `/curate/:style/:block/:bucket` only.
- **F6: MiniBar overlaps mobile AppShell.Footer.** Both pin to `bottom: 0` with `position: fixed`. On mobile, the existing nav footer shows ABOVE MiniBar in DOM order, so they visually overlap. Fix path: media query in `MiniBar.module.css` to set `bottom: var(--mobile-footer-height)` on mobile, or stack via z-index. Acceptable for F6; design polish in F10.
- **F6: PlaybackProvider's `track.current` is set inside `play()` and `advance()`, not from SDK `player_state_changed`.** SDK only knows the Spotify track URI; CLOUDER's `PlaybackTrack` shape (id, title, artists, duration_ms, spotify_id, cover_url) lives in the bound queue. The provider sets `track.current = queue.tracks[targetIdx]` when initiating a play. `clearQueue()` clears it. The `player_state_changed` listener only updates `positionMs` / `durationMs`. (Caught during T33 integration testing — without this, MiniBar's `track.current !== null` gate never opens.)
- **F6: Mantine 9 `Title` does not support a `truncate` prop.** Forwarding `truncate` to `Title` produces a React unknown-prop warning. PlayerCard's title uses inline `overflow: hidden; textOverflow: ellipsis; whiteSpace: nowrap` instead. Mantine `Text` still supports `truncate` — that one works.
- **F6: `spotify-web-playback-sdk` types via triple-slash reference.** `tsconfig.app.json` has an explicit `types: [...]` allowlist that does not auto-include the new types package. The reference `/// <reference types="spotify-web-playback-sdk" />` lives at the top of `frontend/src/types/spotify-sdk.d.ts` to make `Spotify.Player` globally reachable without expanding the tsconfig types array (which would surface SDK types project-wide).
- **`QueueSource` is now a discriminated union.** Variants: `{ type: 'bucket', blockId, bucketId }` (curate) and `{ type: 'category', categoryId, styleId }` (category player). Always narrow on `source.type` before reading variant-specific fields.
- **MiniBar and LeaveContextDialog have been deleted.** Both Curate and Category players own their routes; navigating away calls `controls.clearQueue()` and there is no global mini-player anymore. `hasPlayerCard(pathname)` now matches both `/curate/:style/:block/:bucket` AND `/categories/:styleId/:id`.
- **`useCategoryTracks` cache key includes `fresh`.** Key tuple is `['categories', 'tracks', id, search, sort, order, tagJoin, tagMatch, fresh]`. Index 8 = boolean `fresh`. Mutations that affect playlist membership (e.g. `useAddTracksToPlaylist`) MUST iterate ALL `['categories', 'tracks']` queries and patch BOTH `fresh=true` (shrink) and `fresh=false` (mark `used_in_playlist=true`) variants.
- **Fullscreen mobile player at `/categories/:styleId/:id/player`.** Back navigation goes to the parent `CategoryDetailPage` route. `useCategoryPlayerQueue.clearQueue` fires on unmount of CategoryPlayerPage (and on unmount of CategoryDetailPage) — so navigating between detail ↔ player keeps the queue alive only if both pages re-bind (they do, because both call `useCategoryPlayerQueue` with the same default fresh-on view).
- **Playlist hotkeys map index 0..9 → `Digit1`..`Digit9, Digit0`.** Index 9 = `Digit0` (NOT `Digit0` for index 0). Hotkey `U` triggers undo. Tag operations have NO hotkey by design.
- **Category player undo stack is depth 1.** `undoStack.push` REPLACES any prior entry. Notification `notifications.hide('category-player-undo')` runs from `useEffect(entry == null)` in `CategoryPlayerPanel` to keep the toast surface in sync. Server-side undo for "Remove from category" is not implemented — that operation pushes a stub-undo placeholder; full re-add requires a server endpoint we haven't built.
- **`?fresh=1|0` on `GET /categories/:id/tracks` is opt-in on the server.** Absent → no filter (backward compatible). The new UI ALWAYS sends an explicit value: default on (`fresh=1`) from the toggle, off → `fresh=0`. The server projects `used_in_playlist` on every row regardless.
