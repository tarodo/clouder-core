...
4. **AWS resource prefix is `clouder-prod-*`** (e.g. `clouder-prod-collector-api`), set by `var.project`. A few resources stay `beatport-prod-*` on purpose (renaming them = data loss / needless cascade): the Aurora cluster + its subnet-group/SG/secret, the `raw` ingest bucket, the `analytics-lake` bucket, the Athena workgroup, and the frontend bucket/OAC/CloudFront functions. The Beatport ingest *provider* code (`beatport_client`, `/admin/beatport/ingest`) is also legitimately named — it's the upstream source.
