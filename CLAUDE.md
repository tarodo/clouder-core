...
- **`SpotifyClient` runs three lookups in order on ISRC miss.** When `?q=isrc:` returns 0 items AND `SPOTIFY_METADATA_FALLBACK_ENABLED=true`, the client tries — in order — (1) sibling ISRCs (`±1, ±2` on last digit, closest-first) verified by title+artist gate (NO duration check, since master/radio-edit can differ wildly), then (2) `?q=track:<title> artist:<artist>` metadata search verified by title+artist+duration gate. Both stages normalize titles via `_normalize_title_for_match` (strips `feat. X`, `(...Mix)`, `- Radio Edit`, `- Extended Mix`, etc.) so suffix decoration doesn't tank fuzzy similarity. Per-component thresholds: `title_sim >= SPOTIFY_FUZZY_TITLE_MIN` (default 0.90), `artist_sim >= SPOTIFY_FUZZY_ARTIST_MIN` (default 0.85), `|duration_diff| <= SPOTIFY_FUZZY_DURATION_TOLERANCE_MS` (default 3000ms — metadata stage only). Disabled by default — set `SPOTIFY_METADATA_FALLBACK_ENABLED=true` on `beatport-prod-spotify-search-worker` to activate. `find_tracks_needing_spotify_search` JOINs `clouder_track_artists`/`clouder_artists` and projects `length_ms` so the worker has the metadata to issue the fallback. Logs: `spotify_isrc_neighbour_match` (stage 1 hit), `spotify_metadata_fallback_attempted` / `_match` / `_rejected` (stage 2), `spotify_metadata_fallback_scores` (best title/artist sims observed, emitted on stage-2 reject for tuning).
