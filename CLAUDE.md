...
**Frontend (post-F1, 2026-05-02):**

- **Run `pnpm dev` from `frontend/`**, not from worktree root. Requires `frontend/.env.local` with `VITE_API_BASE_URL=$(cd infra && terraform output -raw api_endpoint)`. SPA on `http://127.0.0.1:5173`, Vite proxies API calls to deployed prod API GW.
