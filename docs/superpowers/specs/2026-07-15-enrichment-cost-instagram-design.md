...
- **Instagram top-up:** if regex found no instagram profile, one targeted Tavily query (`site:instagram.com "<name>"`-style, $0.008) → validate handle, else null.
...
- **Metrics vs baseline:** per-field fill-rate (table above), instagram found-rate + manual spot-check of ~20 handles for correctness, numeric fields sourced-rate, measured cost/run (search calls + credits), latency.
