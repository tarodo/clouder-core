...
| **SPA→API delivery** | The SPA reaches API Gateway only through CloudFront (`api_gw_pure_path_patterns` / `api_gw_spa_aware_path_patterns`, `infra/frontend.tf`) and the Vite dev proxy (`frontend/vite.config.ts`). A **new API prefix (`/v1`) must be added to both**, or the browser `fetch` falls through to the SPA shell and never reaches the gateway. `/v1/*` is a pure API prefix (no SPA-route collision), registered once, covering telemetry + analytics. |
...
| `playback_play` | `track_id`, `position_ms`=0, `duration_ms`, `source` (`"triage_player"`\|`"playlist_player"`\|`"category_player"`) | `frontend/src/features/playback/PlaybackProvider.tsx` — `play()` gains an **explicit `source` argument**: signature becomes `play(idx?, overrideTrack?, source?: PlaybackSource)`, and each initiating panel passes its literal at the call site (the chosen approach). As a safety net `source` **defaults** to a mapping of `queue.source.type` (`bucket→triage_player`, `category→category_player`, `playlist→playlist_player` — `QueueSource` already carries all three variants, `lib/types.ts:32-35`), so an un-passed call still emits a correct value. Emit inside `play()` after the track resolves (the local is named `track`, ~L354). |
...
| `playlist_add` | `track_ids` (**per-track** array of the added ids — sources the funnel `playlisted` step), `playlist_id`, `track_count` (=`selected.size`), `source_category_id` (category the tracks came from when in scope, else null) | `frontend/src/features/playlists/components/AddTracksModal.tsx` — `handleSubmit()` (~L65) → `addMut.mutateAsync` onSuccess (~L72-76). The added ids come from the modal's selection set (`selected`). |
...
| `playlist_publish` | `track_ids` (**per-track** array of the published ids — sources the funnel `published` step), `playlist_id`, `track_count`, `confirm_overwrite`, `skipped_count`, `target` (`"spotify"`\|`"ytmusic"`) | `frontend/src/features/playlists/components/PublishButton.tsx` — `doPublish()` (~L32) onSuccess (~L34-53). `PublishYtMusicButton` is the `ytmusic` variant (same emit, `target:'ytmusic'`). Published ids come from the playlist's current track list at publish time. |
...
4. **`infra/frontend.tf` + `frontend/vite.config.ts`** — register the **`/v1` API prefix** in CloudFront `api_gw_pure_path_patterns` (pure API prefix, no SPA-route collision) **and** the Vite dev proxy. Without this the SDK's `fetch('/v1/telemetry')` falls through to the SPA shell instead of reaching API Gateway. `/v1/*` covers both telemetry and analytics, so it is registered once (here, or in Increment 5 — whichever lands first).
...
- **`fact_funnel_step` model:** a window function over each track's events ordered by `ts_server` assigns `step`/`prev_step` and computes `ms_since_prev = ts - lag(ts) over (partition by track_key order by ts_server)`. `viewed`/`categorized` come from `track_view`/`track_categorized`; `playlisted` and `published` are **unnested from the per-track `track_ids` arrays** on `playlist_add` / `playlist_publish` (one funnel row per track id — Athena `UNNEST`); `ingested` is merged from `clouder_tracks.created_at` (daily catalog export — no live join). This makes Dashboard 3's ingest→publish drop-off and time-to-publish fully derivable in P1.
...
- **`analytics-api` is a standalone Lambda function** with its own least-privilege role (Athena + S3 read on `gold/` + `athena-results/`). It **may share the zip artifact** (like `catalog_export`) but is a **separate function and integration** — it is *not* a dispatch branch in the collector handler, so the collector role never gains Athena/S3-gold read. The five routes — `GET /v1/analytics/{triage,taste,funnel,playback,ops}` — get their **own admin-gated API Gateway integration** to this function (registered via the three-place rule, §5.1, in a new `infra/analytics_routes.tf`). They are **not** added to the collector handler's `_ADMIN_ROUTES` table; instead `analytics-api` enforces the admin check itself on the authorizer context. The `/v1` API prefix is registered once in CloudFront `api_gw_pure_path_patterns` (`infra/frontend.tf`) + the Vite dev proxy (`frontend/vite.config.ts`) so the SPA's `fetch('/v1/analytics/...')` reaches API Gateway (§5.1 step 4); without it the request falls through to the SPA shell. Each route runs a **parameterized, pre-written Athena query** against `gold/` — clients pick a date range + filters, never send raw SQL.
...
1. **Ops-domain log shipping (Dashboard 5):** CloudWatch Lambda logs → S3 needs a subscription filter or scheduled export. **Decision (now first-class):** Phase 1 ships an EventBridge-Scheduler-driven Lambda, **`ops_log_export`**, that pulls the relevant log groups' enrichment/latency metrics to `bronze/ops/`. It is carried through the architecture body: §5 diagram, §9 DAG (parallel sibling of `catalog_export`, before `dbt_run`), §12 cost (5th Lambda line), §13 IAM (own role). Full CloudWatch→Firehose subscription is a P2 nicety. Dashboard 5 starts with enrichment success/latency (from `bronze/ops/`) + pipeline freshness (reads `bronze/events` `dt` — needs no log shipping). **Field/key note (from recon):** structlog applies `EventRenamer("message")` (`logging_utils.py:111`), so the event name lands under the **`message`** key, not `event` — `ops_log_export` must read `message`. And `source_hint` + dispatch counts come from `auto_dispatch.py`, whose callers run in the **`auto_enrich_dispatch_worker`** / `collector-api` log groups — those must be in the pull list alongside the canonicalization/spotify/vendor/label/artist enricher worker groups.
2. **Funnel "ingested" step:** `viewed`/`categorized`/`playlisted`/`published` all come from events — `playlisted`/`published` unnested from the **per-track `track_ids`** arrays on `playlist_add`/`playlist_publish` (§3.2/§8). Only `ingested` is catalog state. **Decision:** derive `ingested` from `clouder_tracks.created_at` via the daily catalog export — no live join. The ordered transition rows (`prev_step`, `ms_since_prev`) are built with a window over per-track events ordered by `ts_server` (§8).
