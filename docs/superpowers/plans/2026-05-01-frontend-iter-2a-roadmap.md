...
| TD-13 | Deep-link / F5 to `/categories/<id>` and `/triage/<id>` returns API JSON instead of the SPA route in prod. CloudFront behaviors route those path patterns to API GW (per the staging-frontend-host design, 2026-05-06) because CF cannot do the Accept-header bypass that Vite proxy uses (`frontend/vite.config.ts:14-30`). In-app react-router navigation still works — only direct URL paste / bookmark / F5 is broken. | New users handed a deep-link URL or a teammate sharing a `/triage/<id>` URL on phone gets raw JSON. Cosmetic during dogfood; blocker for any external sharing. | Two paths: (a) move backend routes under `/api/categories`, `/api/triage` (cleanest, requires API GW route changes + handler dispatch + `apiClient` baseUrl + openapi regen — ~30 min); or (b) add a Lambda@Edge on the two SPA-aware behaviors that switches origin to S3 when `Accept: text/html` (preserves URL shape, adds ~50ms cold-start latency + ongoing cost). Fix before opening up to non-dev users. |
